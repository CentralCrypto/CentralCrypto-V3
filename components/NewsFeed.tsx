...
import { ArrowLeft, ArrowRight, Loader2, Clock, User, AlertCircle, RefreshCw } from 'lucide-react';
...
const NewsFeed: React.FC<NewsFeedProps> = ({ onPostClick, language }) => {
  ...
  const fetchLists = useCallback(async () => {
        try {
            ...
            if (Array.isArray(cats)) {
                dailyId = cats.find((c:any) => c.slug?.includes('noticia') || c.slug?.includes('daily'))?.id;
                editorId = cats.find((c:any) => c.slug?.includes('editor') || c.slug?.includes('featured'))?.id;
                bulletinsId = cats.find((c:any) => c.slug?.includes('boletim') || c.slug?.includes('mini'))?.id;
            }

            // Busca paralela com proteção contra IDs vazios
            const [d, e, b] = await Promise.all([
                fetchMagazinePosts({ categories: dailyId || 0, perPage: 5 }),
                fetchMagazinePosts({ categories: editorId || 0, perPage: 5 }),
                fetchMagazinePosts({ categories: bulletinsId || 0, perPage: 5 })
            ]);
            ...
            console.error("NewsFeed specifics error", e);
            // Se as listas específicas falham, não travamos o app, o feed principal pode funcionar
        } finally { 
  ...
  const fetchFeed = useCallback(async () => {
        ...
        try {
            ...
            if (!error) setError(e.message || 'Falha ao carregar feed principal.');
        }
  ...
  if (error && mainFeed.length === 0 && dailyNews.length === 0) return (
    <div className="w-full bg-tech-950 border border-tech-800 p-12 rounded-xl text-center shadow-2xl">
        ...
        <h4 className="text-gray-200 font-bold text-xl mb-2">Erro na Magazine</h4>
        <p className="text-gray-400 text-sm mb-6">{error}</p>
        <button onClick={handleRetry} className="bg-tech-800 hover:bg-tech-700 text-[#dd9933] font-bold px-6 py-2 rounded-lg border border-[#dd9933]/30 flex items-center gap-2 mx-auto transition-all">
            <RefreshCw size={16} /> Recarregar Feed
        </button>
    </div>
  );
    ...
    <div className="w-full h-full bg-tech-950 border border-tech-800 p-6 rounded-xl shadow-2xl transition-colors">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-3 flex flex-col gap-6">
                ...
                {loadingSpecifics ? <div className="animate-pulse space-y-4"><div className="h-40 bg-tech-800 rounded"></div><div className="h-20 bg-tech-800 rounded"></div></div> : (
                  <>
                        ...
                        {dailyNews.slice(2, 5).map(post => (
                            <div onClick={() => onPostClick(post.id)} key={post.id} className="group cursor-pointer flex gap-3 items-start">
                                <div className="w-16 h-16 shrink-0 rounded bg-tech-900 border border-tech-800 overflow-hidden">
                                    <img src={post.featuredImage} className="w-full h-full object-cover opacity-70 group-hover:opacity-100 transition-opacity" alt="" />
                                </div>
                                <div>
                                    <h5 className="text-gray-300 dark:text-[#dd9933] font-medium text-xs leading-snug group-hover:text-[#dd9933] dark:group-hover:text-white line-clamp-2 transition-colors">{decodeHTML(post.titleHtml)}</h5>
                                ...
                                <div onClick={() => onPostClick(post.id)} key={post.id} className="flex flex-col md:flex-row gap-4 border-b border-tech-800/50 pb-6 group cursor-pointer">
                                    ...
                                    <div className="flex-1 flex flex-col justify-center">
                                        ...
                                        <div className="flex items-center gap-3 text-[10px] text-gray-500 font-mono uppercase">
                                            <span className="text-[#dd9933] font-bold">{post.authorName}</span><span>•</span><span>{new Date(post.date).toLocaleDateString(currentLocale, {day: '2-digit', month: 'short', year: 'numeric'})}</span>
                                        </div>
                                    </div>
                                </div>
                            ))
            ...
            <div className="lg:col-span-3">
                ...
                <div className="flex flex-col gap-4">
                    {loadingSpecifics ? <div className="space-y-4 animate-pulse"><div className="h-24 bg-tech-800 rounded"></div><div className="h-24 bg-tech-800 rounded"></div></div> : (
                      bulletins.map(post => (
                        <div onClick={() => onPostClick(post.id)} key={post.id} className="group cursor-pointer bg-tech-900 border border-tech-800 hover:border-[#dd9933] p-3 rounded-lg shadow-lg transition-all hover:-translate-y-1">
                            ...
                            <div className="flex items-center text-[10px] text-gray-500 gap-1"><User size={10} /> {post.authorName} <span className="mx-1">•</span> <Clock size={10} /> {new Date(post.date).toLocaleDateString(currentLocale)}</div>
                        </div>
                      ))
                    )}
                </div>
