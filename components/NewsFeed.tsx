...
const NewsFeed: React.FC<NewsFeedProps> = ({ onPostClick, language }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
        setCarouselIndex(prev => (prev + 1) % carouselFiltered.length);
    }, 6000);
                            ...
                            <div className="relative w-full h-[350px] rounded-xl overflow-hidden group shadow-2xl border border-tech-700" onClick={() => onPostClick(carouselFiltered[carouselIndex].id)}>
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-8 cursor-pointer">
                                    ...
                                    <h2 className="text-3xl font-black text-white dark:text-[#dd9933] leading-tight drop-shadow-md mb-4 hover:text-[#dd9933] dark:hover:text-white transition-colors">
                                        {decodeHTML(carouselFiltered[carouselIndex].titleHtml)}
                                    </h2>
                                    <div className="flex items-center gap-4 text-xs text-gray-300 font-mono">
                                        <span className="flex items-center gap-1 font-bold text-[#dd9933]"><User size={12}/> {carouselFiltered[carouselIndex].authorName}</span>
                                        <span className="flex items-center gap-1"><Clock size={12}/> {new Date(carouselFiltered[carouselIndex].date).toLocaleDateString(currentLocale)}</span>
                                    </div>
                                </div>
                            </div>
                        )}
