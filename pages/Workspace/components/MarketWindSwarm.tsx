...
const MarketWindSwarm = ({ language, onClose }: MarketWindSwarmProps) => {
  ...
  useEffect(() => {
      ...
      const newParticles = topCoins.map(coin => {
          ...
          let baseRadius = 8;
          if (chartMode === 'performance') {
             baseRadius = (15 + (Math.log10(radiusVal) - logMinR) / (logMaxR - logMinR || 1) * 55); 
          } else {
             baseRadius = (15 + (radiusVal - minR) / (maxR - minR || 1) * 55);
          }
          ...
          if (existing) {
              ...
              return {
                  id: coin.id,
                  x: containerRef.current ? containerRef.current.clientWidth / 2 + (Math.random()-0.5)*200 : 0,
                  y: containerRef.current ? containerRef.current.clientHeight / 2 + (Math.random()-0.5)*200 : 0,
  ...
  useEffect(() => {
    ...
    animationLoopFn.current = () => {
        try {
            ...
            const physicsSpeed = isFreeMode ? animSpeedRaw * 3.0 : animSpeedRaw * 1.5;
            ...
            if (!isFreeMode) {
                ...
                for (let i = 0; i <= xSteps; i++) {
                    ...
                    if (px >= 0 && px <= width) {
                        ...
                        ctx.fillText(label, px, height - 30);
                    }
                }
            ...
            // --- COLLISION RESOLUTION (Free Mode) ---
            if (isFreeMode) {
                for (let i = 0; i < particles.length; i++) {
                    ...
                    for (let j = i + 1; j < particles.length; j++) {
                        ...
                        if (distSq < minDist * minDist) {
                            ...
                            const overlap = (minDist - dist) * 0.5; // Split overlap
                            const nx = dx / dist;
                            const ny = dy / dist;
            ...
            for (let i = 0; i < particles.length; i++) {
                const p = particles[i];
                const targetR = p.baseRadius * Math.pow(zoomRef.current, 0.4); 
                ...
                if (isFreeMode) {
                    if (draggedParticleRef.current?.id !== p.id) {
                        ...
                        // Keep alive
                        const minMove = physicsSpeed * 10;
                        if (Math.abs(p.vx) + Math.abs(p.vy) < minMove) {
                            p.vx += (Math.random() - 0.5) * minMove;
                            p.vy += (Math.random() - 0.5) * minMove;
                        }
                    }
                } else {
                    ...
                    const floatAmp = 5 * physicsSpeed;
                    const floatX = Math.sin(now * 0.002 * physicsSpeed + p.phase) * floatAmp;
                    const floatY = Math.cos(now * 0.003 * physicsSpeed + p.phase) * floatAmp;
                ...
                if (trailLength > 0) {
                    ...
                    if (!last || Math.abs(p.x - last.x) > 2 || Math.abs(p.y - last.y) > 2) {
                        p.trail.push({ x: p.x, y: p.y, age: 1.0 });
                    } else {
                        // Decay static trails
                        p.trail.forEach(t => t.age -= 0.05);
                    }
                    p.trail = p.trail.filter(t => t.age > 0).slice(-trailLength);
                } else {
                    p.trail = [];
                }
            ...
            if (p) drawTooltip(ctx, p, width, height);
        } catch (e) {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (lastMousePosRef.current && draggedParticleRef.current) {
        ...
        draggedParticleRef.current.vx = dx * 20; 
        draggedParticleRef.current.vy = dy * 20;
    }
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 z-[2000] bg-white dark:bg-[#0b0f14] text-gray-900 dark:text-white flex flex-col overflow-hidden">
