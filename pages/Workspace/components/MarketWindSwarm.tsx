...
import { Search, XCircle, Settings, Droplets, FastForward, Activity, Wind, X, Atom, Scaling, CandlestickChart, Coins } from 'lucide-react';
...
const MarketWindSwarm = ({ language, onClose }: MarketWindSwarmProps) => {
  ...
  useEffect(() => {
      if (!isFreeMode) {
          particlesRef.current.forEach(p => {
              p.isReturning = true; p.startX = p.x; p.startY = p.y; p.animProgress = 0;
          });
      }
  }, [isFreeMode]);
  ...
  useEffect(() => {
    ...
    const found = particlesRef.current.find(p => p.coin.name.toLowerCase().includes(searchTerm.toLowerCase()) || p.coin.symbol.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        setCoins(currentCoins => { previousCoinsRef.current = currentCoins; return data; });
        setStatus('running');
  ...
  useEffect(() => {
    ...
    animationLoopFn.current = () => {
        ...
        if (!isFreeMode && mappings) {
          ...
          for (let i = 0; i <= 5; i++) { // Y-Axis (Volume)
            const val = Math.pow(10, Math.log10(minY) + i * (Math.log10(maxY) - Math.log10(minY)) / 5);
            const y = mappings.mapY(val);
            if (y > pad * 0.5 && y < height - pad * 0.5) { ctx.beginPath(); ctx.moveTo(mappings.mapX(minX) - 5, y); ctx.lineTo(mappings.mapX(maxX), y); ctx.stroke(); ctx.fillText(`${formatCompact(val)}`, 15, y + 3); }
          }
          if (chartMode === 'performance') { // X-Axis (Perf %)
              for (let i = 0; i <= 10; i++) {
                const val = minX + i * (maxX - minX) / 10; const x = mappings.mapX(val);
                if (x > pad * 0.5 && x < width - pad * 0.5) { ctx.beginPath(); ctx.moveTo(x, mappings.mapY(minY)); ctx.lineTo(x, mappings.mapY(maxY) + 5); ctx.stroke(); ctx.textAlign = 'center'; ctx.fillText(`${val.toFixed(1)}%`, x, height - pad + 15); }
              }
          } else { // X-Axis (Market Cap)
              for (let i = 0; i <= 5; i++) {
                const val = Math.pow(10, Math.log10(minX) + i * (Math.log10(maxX) - Math.log10(minX)) / 5); const x = mappings.mapX(val);
                if (x > pad * 0.5 && x < width - pad * 0.5) { ctx.beginPath(); ctx.moveTo(x, mappings.mapY(minY)); ctx.lineTo(x, mappings.mapY(maxY) + 5); ctx.stroke(); ctx.textAlign = 'center'; ctx.fillText(`${formatCompact(val)}`, x, height - pad + 15); }
              }
          }
          ...
          ctx.save(); ctx.translate(25, height / 2); ctx.rotate(-Math.PI / 2); ctx.fillText('Volume 24h (Log)', 0, 0); ctx.restore();
        }

        particles.forEach(p => {
            if (p.isReturning && p.animProgress < 1) {
                p.animProgress = Math.min(1, p.animProgress + delta * 0.7); // SLOWER RETURN
            ...
            p.trail.push({ x: p.x, y: p.y }); while (p.trail.length > trailLength) p.trail.shift();
        ...
        if (particleForTooltip) {
            ...
            ctx.font = 'bold 14px Inter'; ctx.fillStyle = '#dd9933'; ctx.fillText(p.coin.symbol.toUpperCase(), ttX + 75, ttY + 62);
            ...
            dataRows.forEach((d, i) => {
              const xPos = ttX + 15 + (i % 2) * (ttWidth/2);
              const yPos = ttY + 95 + Math.floor(i/2) * 28;
              ctx.fillStyle = isDark ? '#999' : '#666'; ctx.fillText(d.label, xPos, yPos);
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    for (let i = particlesRef.current.length - 1; i >= 0; i--) {
        ...
        const hitRadius = Math.max(drawRadius * 1.4, 16); // HOVER FIX
    ...
    <div ref={containerRef} className="fixed inset-0 z-[2000] bg-white dark:bg-[#0b0f14] text-gray-900 dark:text-white flex flex-col">
      ...
      {settingsOpen && (
          <div className="absolute top-24 right-4 bg-white/90 dark:bg-black/80 p-4 rounded-lg border border-gray-200 dark:border-white/10 backdrop-blur-md w-72 z-20 space-y-4 animate-in fade-in slide-in-from-top-4 shadow-xl">
              <div className="flex justify-between items-center"><label className="text-xs font-bold flex items-center gap-2"><Atom size={14} /> Modo Livre</label>{/* ... */}</div>
