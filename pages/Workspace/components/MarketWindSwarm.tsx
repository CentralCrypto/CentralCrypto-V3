...
// Fix: Added 'error' to the Status type to handle API error states.
...
// Fix: Defined the missing 'clamp' utility function.
...
const MarketWindSwarm = ({ language, onClose }: MarketWindSwarmProps) => {
  ...
  const loadData = useCallback(async (isInitial = false) => {
    ...
    try {
      ...
      console.error("Swarm: data fetch failed.", error);
  ...
  useEffect(() => {
    ...
    particlesRef.current = topCoins.map(coin => {
      ...
      if (existing) {
        ...
        existing.animProgress = 0; // Reset animation
        existing.coin = coin; // Update coin data
        existing.radius = 8 + (Math.log10(coin.market_cap || 1) - minLogMc) / (maxLogMc - minLogMc || 1) * 42;
        existing.color = (coin.price_change_percentage_24h || 0) > 0 ? '#089981' : '#f23645';
        return existing;
      } else {
        const radius = 8 + (Math.log10(coin.market_cap || 1) - minLogMc) / (maxLogMc - minLogMc || 1) * 42;
        return {
          id: coin.id,
          x: Math.random() * canvas.width, y: Math.random() * canvas.height,
          startX: Math.random() * canvas.width, startY: Math.random() * canvas.height,
          targetX: 0, targetY: 0, // Will be set in animation loop
          animProgress: 1, // Start at final position
          radius,
  ...
  const animate = useCallback(() => {
    ...
    // --- Map Targets ---
    const xValues = particles.map(p => p.coin.price_change_percentage_24h || 0);
    const yValues = particles.map(p => p.coin[yAxisMode] || 1);
    const minX = Math.min(...xValues), maxX = Math.max(...xValues);
    const minY = Math.min(...yValues), maxY = Math.max(...yValues);
    const logMinY = Math.log10(minY), logMaxY = Math.log10(maxY);
    ...
    particles.forEach(p => {
        p.targetX = mapX(p.coin.price_change_percentage_24h || 0);
        p.targetY = mapY(p.coin[yAxisMode] || 1);
    });
    ...
    for (let i = 0; i <= 5; i++) {
      ...
      ctx.beginPath();
      ctx.moveTo(pad - 5, y); ctx.lineTo(width - pad, y);
      ctx.stroke();
      const val = (yAxisMode === 'price_change_percentage_24h')
          ? maxY - i * (maxY - minY) / 5
          : Math.pow(10, logMaxY - i * (logMaxY - logMinY) / 5);
    ...
    for (let i = 0; i <= 10; i++) {
      ...
      ctx.beginPath();
      ctx.moveTo(x, pad); ctx.lineTo(x, height - pad + 5);
      ctx.stroke();
      const val = minX + i * (maxX - minX) / 10;
      ctx.textAlign = 'center';
      ctx.fillText(`${val.toFixed(1)}%`, x, height - pad + 15);
    }
    ...
    particles.forEach(p => {
        ...
        if (p.animProgress > 0 && p.animProgress < 1) {
            ...
            ctx.lineTo(p.x, p.y);
            ctx.strokeStyle = 'rgba(221, 153, 51, 0.4)';
            ctx.lineWidth = 2;
            ctx.stroke();
        }
        ...
        if (selectedParticle?.id === p.id || hoveredParticle?.id === p.id) {
            ...
            ctx.lineWidth = selectedParticle?.id === p.id ? 4 : 2;
            ctx.stroke();
        }
    });
    ...
    if (hoveredParticle && !selectedParticle) {
        ...
        const ttWidth = 200, ttHeight = 100;
        let ttX = p.x - ttWidth / 2;
        let ttY = p.y - p.radius - ttHeight - 10;
    ...
    animationFrameId.current = requestAnimationFrame(animate);
  }, [animSpeed, selectedParticle, hoveredParticle, yAxisMode, isDark]);
    ...
    <div ref={containerRef} className="fixed inset-0 z-[2000] bg-white dark:bg-[#0b0f14] text-gray-900 dark:text-white flex flex-col">
      ...
      <div className="flex justify-between items-start p-4 z-10 bg-white/80 dark:bg-black/50 backdrop-blur-sm border-b border-gray-200 dark:border-white/10 shrink-0">
        <div className="flex items-center gap-4">
            ...
            <div>
                <h3 className="text-xl font-black uppercase tracking-wider">CentralCrypto WindSwarm</h3>
                <p className="text-xs text-gray-500 dark:text-gray-400 font-bold">{status === 'demo' ? 'MODO DEMO' : 'Live Market Data'}</p>
            </div>
            ...
            <div className="flex items-center gap-2 bg-gray-100 dark:bg-black/50 p-2 rounded-lg border border-gray-200 dark:border-white/10">
                ...
                <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Buscar ativo..." className="bg-transparent outline-none text-sm w-48 text-gray-900 dark:text-white" />
      ...
      {settingsOpen && (
          <div className="absolute top-24 right-4 bg-white/90 dark:bg-black/80 p-4 rounded-lg border border-gray-200 dark:border-white/10 backdrop-blur-md w-64 z-20 space-y-4 animate-in fade-in slide-in-from-top-4 shadow-xl">
              <div className="space-y-2"><label className="text-xs font-bold flex items-center gap-2"><FastForward size={14} /> Velocidade</label><input type="range" min="0.1" max="2" step="0.1" value={animSpeed} onChange={e => setAnimSpeed(parseFloat(e.target.value))} className="w-full accent-[#dd9933]" /></div>
