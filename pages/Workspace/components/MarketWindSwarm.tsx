...
// FRICTION: deixa parar mesmo
const GAME_LINEAR_DAMP = 0.968; // mais atrito = para mais rápido
const GAME_STOP_EPS = 2.2;      // abaixo disso zera

// FREE MODE physics
const FREE_LINEAR_DAMP = 0.992;
const FREE_MAX_SPEED = 420;
const FREE_REPULSE = 0.85; // empurra pra fora no overlap
...
// LOG scale for bubble size in valuation mode (ocupando melhor sem explodir)
...
targetRadius = 16 + tlog * 74; // ocupa melhor
}

// game layout força seus tamanhos próprios
if (!isGameMode) {
...
}, [getCoinPerfPct, numCoins, sizeMetricPerf, isGameMode, sizeLogScale]);

// ======== map projection helper (NO SNAP, only targets) ========
...
// set transition from current to target
p.mapFromX = p.x;
p.mapFromY = p.y;
p.mapToX = tx;
p.mapToY = ty;
...
// update map targets on resize (smoothly)
if (!isGameMode) {
computeMapTargets();
}
};
...
// initial map targets (smooth)
if (!isGameMode) {
computeMapTargets();
}
}, [coins, numCoins, recomputeStatsAndTargets, chartMode, isGameMode, computeMapTargets]);

// when chartMode or timeframe changes: DO NOT rebuild, only update targets/colors + new map transition
useEffect(() => {
...
const maxPull = 210;
aimingRef.current.pull = clamp((held / 850) * maxPull, 0, maxPull);
return;
}
...
const pullNorm = clamp(pull / 210, 0, 1);

// mais forte (ponta a ponta)
const basePower = 4200;
...
<div>• Clique para mirar, segure para carregar força.</div>
...
<div>• Bolhas flutuam livres.</div>
<div>• Colisão newtoniana (não atravessa).</div>
<div>• Market Cap / Variação apenas ajusta TAMANHO e COR.</div>
</>
);
}
...
// MAP axes (only when mapped mode and not free)
...
// pocket detection + falling animation
for (const p of particles) {
if (p.isFalling) continue;
if (p.isFixed) continue;
...
// MAPPED MODE: smooth transition (NO PISCAR)
if (!statsRef.current) {
...
// transition progress
const t0 = p.mapT ?? 1;
const t1 = clamp(t0 + dt / 0.55, 0, 1); // ~0.55s transition
p.mapT = t1;
...
// UI helpers
// =======================
const effectiveNumCoins = useMemo(() => {
if (isGameMode) return Math.min(50, numCoins);
return numCoins;
}, [isGameMode, numCoins]);
...
{/* MOVED: num coins dropdown to header (before search) */}
...
{/* FREE MODE toggle (hidden in game) */}
{!isGameMode && (
