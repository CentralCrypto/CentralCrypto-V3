...
const computeSparkChange = (coin: any, tf: Timeframe) => {
  ...
  return { pct, absPct: Math.abs(pct) };
};
...
const CryptoMarketBubbles = ({ language, onClose, isWidget = false, item }: CryptoMarketBubblesProps) => {
  ...
  useEffect(() => {
      if (gameWon) {
          ...
          // Toca vitória se som estiver habilitado
          if (soundEnabledRef.current && sfxVitoriaRef.current) {
  ...
  useEffect(() => {
    if (isGameMode) {
      ...
      // Removed generic bgMusic play here, now handled by specific useEffect
      ...
      // Music stop handled by specific useEffect
    }
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    // User interaction enables audio context if needed, handled by browser
    if (isGameMode && bgMusicRef.current && bgMusicRef.current.paused && soundEnabled && !gameWon && !gameOver) {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (rs.isGameMode) {
        ...
        const TOTAL_PAD = FLOOR_MARGIN + RAIL_TOTAL; 
        ...
        ctx.beginPath();
        ctx.roundRect(woodX, woodY, woodW, woodH, 20); 
        ctx.fill();
          ...
          for (const p of [...particles]) {
            ...
            if (t >= 1) {
              if (!p.scoreCounted) {
                  ...
                  if (wasCue) {
                    setGameOver(true);
                    playSound(sfxFallRef.current); 
                    setTimeout(() => playSound(sfxGameOverRef.current), 800); 
                  } else {
      ...
      if (rs.isGameMode) {
        ...
        if (cueBall && now >= cueHideUntilRef.current) {
          ...
          ctx.beginPath();
          ctx.moveTo(buttX + perpX * buttW, buttY + perpY * buttW); 
