...
interface Particle {
  ...
  // Game Logic Flags
  isProcessed?: boolean; // Critical for atomic scoring
...
const CryptoMarketBubbles = ({ language, onClose, isWidget = false, item }: CryptoMarketBubblesProps) => {
  ...
  // IMPORTANT: Cache keyed by COIN ID (string)
  ...
  // Refs for tracking game state without re-renders inside physics loop
  ...
  const setupGameLayout = useCallback(() => {
    ...
    for (const p of particlesRef.current) {
      ...
      p.isFixed = false;
      p.isFalling = false;
      p.isProcessed = false; // Reset atomic score flag
      p.fallT = 0;
      p.fallPocket = null;
      p.mapT = 1;
    }
    ...
    // Reset turn stats
    turnStatsRef.current = { active: false, startCount: 0, consecutiveHits: 0, consecutiveMisses: 0 };
  ...
  // COIN IMAGE LOADING WITH LOCAL CACHE FALLBACK (Updated)
  useEffect(() => {
    ...
    for (const c of topCoins) {
      if (c?.id && !imageCache.current.has(c.id)) {
        const img = new Image();
        // Use Logo Service to find best URL (Local Cache Priority)
    ...
    const newParticles: Particle[] = topCoins.map(coin => {
      ...
      return {
        ...
        fallPocket: null,
        mapT: 0,
        isProcessed: false // Initialize atomic flag
      };
    });
