...
const CalendarWidget: React.FC<{ item: DashboardItem, language?: Language }> = ({ item, language = 'pt' }) => {
    ...
    const [impactFilter, setImpactFilter] = useState<'ALL' | 'High' | 'Medium' | 'Low' | 'Holiday'>('ALL');
    ...
    const getImpactColor = (impact: string) => {
        ...
        if (impact === 'Holiday') return 'bg-gray-400';
    ...
    return (
        <div className="h-full flex flex-col relative bg-white dark:bg-[#2f3032] p-4 transition-colors">
             <div className="flex flex-col gap-4 mb-6 shrink-0 border-b border-gray-100 dark:border-slate-700/50 pb-4">
                ...
                <div className="flex justify-between items-center w-full">
                     <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mr-2">Impacto:</span>
                        <button onClick={() => setImpactFilter(prev => prev === 'High' ? 'ALL' : 'High')} className={`w-3 h-3 rounded-full bg-red-500 ring-2 ring-offset-2 dark:ring-offset-[#2f3032] transition-all ${impactFilter === 'High' ? 'ring-red-500 scale-110' : 'ring-transparent opacity-50 hover:opacity-100'}`} title="Alto Impacto"></button>
                        <button onClick={() => setImpactFilter(prev => prev === 'Medium' ? 'ALL' : 'Medium')} className={`w-3 h-3 rounded-full bg-orange-500 ring-2 ring-offset-2 dark:ring-offset-[#2f3032] transition-all ${impactFilter === 'Medium' ? 'ring-orange-500 scale-110' : 'ring-transparent opacity-50 hover:opacity-100'}`} title="Médio Impacto"></button>
                        <button onClick={() => setImpactFilter(prev => prev === 'Low' ? 'ALL' : 'Low')} className={`w-3 h-3 rounded-full bg-yellow-500 ring-2 ring-offset-2 dark:ring-offset-[#2f3032] transition-all ${impactFilter === 'Low' ? 'ring-yellow-500 scale-110' : 'ring-transparent opacity-50 hover:opacity-100'}`} title="Baixo Impacto"></button>
                        <button onClick={() => setImpactFilter(prev => prev === 'Holiday' ? 'ALL' : 'Holiday')} className={`w-3 h-3 rounded-full bg-gray-400 ring-2 ring-offset-2 dark:ring-offset-[#2f3032] transition-all ${impactFilter === 'Holiday' ? 'ring-gray-400 scale-110' : 'ring-transparent opacity-50 hover:opacity-100'}`} title="Feriado"></button>
                     </div>
                         ...
                         {filteredEvents.map((e, i) => {
                             ...
                             return (
                                 <div key={i} className={`flex items-center gap-5 ${rowPadding} rounded-2xl hover:bg-gray-100 dark:hover:bg-white/5 border border-transparent hover:border-gray-100 dark:hover:border-slate-700 transition-all shadow-md bg-white dark:bg-[#1a1c1e]/30 group`}>
                                     ...
                                     <div className={`w-2.5 ${item.isMaximized ? 'h-20' : 'h-12'} rounded-full shrink-0 ${getImpactColor(e.impact)} shadow-lg`} title={`Impacto: ${e.impact}`}></div>
