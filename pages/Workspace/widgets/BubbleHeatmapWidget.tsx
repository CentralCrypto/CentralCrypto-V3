        ...
        <div className="h-full flex flex-col bg-black relative overflow-hidden">
                ...
                <div className="fixed inset-0 z-[2000] bg-black flex flex-col animate-in fade-in duration-600">
                    <div className="flex justify-between items-center px-10 py-6 bg-[#0a0b0c] border-b border-white/10 shrink-0">
                        <div className="flex items-center gap-6">
                            <div className="p-3 bg-tech-800 rounded-xl border border-[#dd9933]/60 shadow-2xl shadow-orange-500/10">
                                <CircleDashed size={34} className="text-[#dd9933]" />
                            </div>
                            <div>
                                <h2 className="text-4xl font-black text-white uppercase tracking-[0.2em]">{mode === 'marketCap' ? 'Bubbles Marketcap' : 'Bubbles Variação'}</h2>
                            </div>
                        </div>
                        <button onClick={closeFullscreen} className="p-4 bg-white/5 hover:bg-red-500/20 text-gray-400 hover:text-red-500 rounded-full transition-all border border-white/10 hover:border-red-500/60 group">
                            <X size={38} className="group-hover:rotate-90 transition-transform duration-300" />
                        </button>
                    </div>
                    {ChartArea}
                    <div className="h-28 bg-[#0a0b0c] border-t border-white/5 flex items-center justify-center px-10 gap-8">
                        <div className="flex gap-6">
                            {LIMIT_OPTIONS.map(opt => (
                                <button 
                                    key={opt}
                                    onClick={() => { setCoinLimit(opt); setChartVersion(v => v + 1); }}
                                    className={`px-10 py-4 rounded-full text-sm font-black transition-all border ${coinLimit === opt ? 'bg-[#dd9933] text-black border-[#dd9933] shadow-2xl shadow-orange-500/30 scale-110' : 'bg-white/5 text-gray-500 border-white/10 hover:text-white'}`}
                                >
                                    {opt} ATIVOS
                                </button>
                            ))}
            ...
            {!isFullscreen && (
                <>
                    <div className="flex flex-col z-30 border-b border-white/10 bg-[#0a0b0c]/98 backdrop-blur-2xl shrink-0">
                        <div className="flex justify-between items-center px-4 py-3.5">
                            <div className="flex items-center gap-3">
                                <CircleDashed size={22} className="text-[#dd9933]" />
                                <div className="flex flex-col">
                                    <span className="text-sm font-black uppercase text-white tracking-tight leading-none">Crypto Bubbles</span>
                                    <span className="text-[9px] font-black text-gray-600 uppercase mt-1 tracking-widest">Smart Selection Top Movers</span>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                <div className="relative group">
                                    <select 
                                        ...
                                        className="bg-white/5 border border-white/10 text-xs font-black text-gray-400 rounded-lg px-3 py-2 outline-none hover:text-white hover:border-[#dd9933] transition-all cursor-pointer appearance-none pr-8 shadow-inner"
                                    >
                                        {LIMIT_OPTIONS.map(opt => (
                                ...
                                <button onClick={() => { loadData(); setChartVersion(v => v + 1); }} className={`p-2 hover:text-[#dd9933] transition-colors rounded ${isLoading ? 'animate-spin' : ''}`}>
                    ...
                    <div className="h-16 bg-[#0a0b0c] border-t border-white/5 flex flex-col justify-center px-6 z-20 shrink-0">
                         <div className="flex flex-col max-w-lg mx-auto w-full gap-2">
                            <div className="h-4 w-full flex border border-white/5 rounded-full overflow-hidden shadow-inner">
                                {[0, 1, 2, 3, 4].map(idx => (
                                    <button 
                                        key={idx} 
                            ...
                            <div className="flex justify-between w-full text-[9px] font-black text-gray-700 uppercase tracking-widest">
                                <span> BEARISH</span>
                                <span>QUEDA</span>
                                <span>NEUTRO</span>
                                <span>ALTA</span>
