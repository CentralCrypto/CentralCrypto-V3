...
export const getMainBoardItems = (): DashboardItem[] => [
  ...
  { id: `main-bubbles`, type: WidgetType.BUBBLE_HEATMAP, title: 'Crypto Bubbles (Physics)', symbol: 'MARKET' },
];
...
export const generateDefaultLayout = (items: DashboardItem[], isMain: boolean = false): Layout[] => {
  if (isMain) {
    return [
        ...
        { i: items[9].id, x: 0, y: 6.2, w: 10, h: 6.2, minW: 4, minH: 4 }, // HEATMAP (Left Half)
        { i: items[10].id, x: 10, y: 6.2, w: 10, h: 6.2, minW: 4, minH: 4 }, // BUBBLES (Right Half)
    ];
  }
