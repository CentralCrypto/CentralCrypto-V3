...
export interface MacdAvgData { averageNMacd: number; bullishPercentage: number; bearishPercentage: number; yesterdayNMacd: number; days7Ago: number; days30Ago: number; }
...
export interface MacdTrackerPoint {
  ...
  marketCap: number;
  logo?: string;
  macd: Record<string, { nmacd: number; signal?: number; hist?: number }>;
}
...
export const fetchRsiTrackerHist = async (): Promise<RsiTrackerPoint[]> => {
  ...
  return items.map((p: any) => {
    ...
    const id = p.id || symbol.toLowerCase();
...
const MACD_TRACKER_TTL_MS = 60_000;
let macdTrackerCacheTs = 0;
let macdTrackerCache: MacdTrackerPoint[] = [];
let macdTrackerInFlight: Promise<MacdTrackerPoint[]> | null = null;
...
export const fetchMacdAverage = async (): Promise<MacdAvgData | null> => {
  ...
  return {
      averageNMacd: safeNum(root?.averageNMacd, 0),
      bullishPercentage: safeNum(root?.bullishPercentage, 50),
      bearishPercentage: safeNum(root?.bearishPercentage, 50),
      yesterdayNMacd: safeNum(root?.yesterdayNMacd, 0),
      days7Ago: safeNum(root?.days7Ago, 0),
      days30Ago: safeNum(root?.days30Ago, 0)
  };
};

export const fetchMacdTracker = async (opts?: { force?: boolean; ttlMs?: number }): Promise<MacdTrackerPoint[]> => {
  const now = Date.now();
  const force = Boolean(opts?.force);
  const ttlMs = typeof opts?.ttlMs === 'number' && isFinite(opts.ttlMs) ? opts!.ttlMs! : MACD_TRACKER_TTL_MS;

  if (!force && macdTrackerCache.length > 0 && (now - macdTrackerCacheTs) < ttlMs) return macdTrackerCache;
  if (!force && macdTrackerInFlight) return macdTrackerInFlight;

  macdTrackerInFlight = (async () => {
      const data = await fetchWithFallback(getCacheckoUrl(ENDPOINTS.cachecko.files.macdTracker));
      let rawItems: any[] = [];
      if (Array.isArray(data)) {
          rawItems = (data[0]?.data && Array.isArray(data[0].data)) ? data[0].data : data;
      } else if (data && data.data) {
          rawItems = Array.isArray(data.data) ? data.data : [];
      }

      macdTrackerCache = rawItems.map((i: any) => ({
          id: i.id || i.symbol?.toLowerCase() || '',
          symbol: i.symbol,
          name: i.name,
          price: safeNum(i.current_price || i.price, 0),
          change24h: safeNum(i.price_change_percentage_24h || i.price24h, 0),
          marketCap: safeNum(i.market_cap || i.marketCap, 0),
          logo: i.image || i.logo,
          macd: i.macd || {}
      })) as MacdTrackerPoint[];

      macdTrackerCacheTs = Date.now();
      return macdTrackerCache;
  })().finally(() => {
      macdTrackerInFlight = null;
  });

  return macdTrackerInFlight;
};
...
export const fetchMacdTablePage = async (args: {
  ...
  ascendingOrder?: boolean;
  filterText?: string;
}): Promise<{ items: MacdTrackerPoint[]; page: number; totalPages: number; totalItems: number }> => {
  const page = Math.max(1, Math.floor(args.page || 1));
  const limit = Math.max(1, Math.min(500, Math.floor(args.limit || 50)));
  const sort = args.sort || 'nmacd';
  ...
  const sorted = [...filtered].sort((a, b) => {
      ...
      } else if (sort.startsWith('macd') || sort === 'nmacd') {
          // If sort key is like 'macd1h', extract tf, else use provided tf
          let targetTf = tf;
          if (sort.startsWith('macd') && sort.length > 4) {
              targetTf = sort.replace('macd', '');
          }
          av = safeNum(a.macd?.[targetTf]?.nmacd, -999);
          bv = safeNum(b.macd?.[targetTf]?.nmacd, -999);
      } else {
          av = safeNum((a as any)[sort], 0);
          bv = safeNum((b as any)[sort], 0);
      }
