...
export const SITE_LOGO_FALLBACK = 'https://centralcrypto.com.br/favicon.ico'; // Fallback final (logo pequeno)
export const COINCAP_CDN = 'https://assets.coincap.io/assets/icons';
export const GITHUB_CDN = 'https://raw.githubusercontent.com/spothq/cryptocurrency-icons/master/128/color';

// Função auxiliar para resolver a melhor URL inicial
export const resolveLogo = (symbol: string, apiImage?: string): string => {
...
/**
 * Helper para extrair array de dados de respostas do n8n/json
 */
const extractDataArray = (raw: any): any[] => {
  ...
  if (raw && typeof raw === 'object') {
    ...
    // Fallback: se for objeto { symbol: 'BTC', ... } retorna [raw]
    if (raw.symbol) return [raw];
  }
  return [];
};
...
export const fetchTopCoins = async (opts?: { force?: boolean; ttlMs?: number }): Promise<ApiCoin[]> => {
  ...
  topCoinsInFlight = (async () => {
    ...
    const coins = extractDataArray(data);
...
export interface MacdTrackerPoint {
  ...
  marketCap: number;
  logo?: string;
  // Corrigido: objeto aninhado para cada timeframe
  macd: Record<string, {
      nmacd: number;
      macd: number;
      histogram: number;
...
export const fetchRsiTrackerHist = async (): Promise<RsiTrackerPoint[]> => {
  ...
  return items.map((p: any) => {
    ...
    return {
      ...
      logo: resolveLogo(symbol, p.image || p.logo),
      rsi: {
...
export const fetchMacdTracker = async (opts?: { force?: boolean }): Promise<MacdTrackerPoint[]> => {
  ...
  return items.map((i: any) => {
      ...
      return {
          ...
          logo: resolveLogo(symbol, i.image || i.logo),
          macd: {
...
export const fetchMacdTablePage = async (args: { page: number; limit: number; sort?: string; timeframe?: string; ascendingOrder?: boolean; filterText?: string; force?: boolean; }): Promise<MacdTablePageResult> => {
    ...
    const sorted = [...filtered].sort((a, b) => {
        let av = 0, bv = 0;
        // Fix: Use correct nested property access
